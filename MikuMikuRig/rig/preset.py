...
class OT_Rig_Preset(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        # If reading preset, try to find source armature (supports selecting _Rig)
        if self.read:
            ...
            # Try to find source armature (works with both source and _Rig selection)
            source_arm = _find_source_armature(context, preset)
            if source_arm is not None and source_arm != context.object:
                # Switch to source armature
...
def _find_source_armature(context, preset):
    """Find source armature from current selection. If _Rig is selected, find original source."""
    obj = context.object
